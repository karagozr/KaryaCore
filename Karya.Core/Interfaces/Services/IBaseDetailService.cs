...
public interface IBaseDetailService<TEntity, TId, TParentId> : IBaseService
{
    IQueryable<TEntity> Query();
    Task<BaseResult<TDto>> ByKey<TDto>(TId key) where TDto : class, ISingleDto, new();

    Task<BaseResult<LoadResult>> Select<TDto>(TParentId parentValue, DataSourceLoadOptionsBase filterDataOptions) where TDto : class, ISelectDto, new();
