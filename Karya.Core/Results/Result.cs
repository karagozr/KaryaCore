...
public record BaseResult
{
    ...
    /// <summary>Semantic message key (see <see cref="MessageCodes"/>); localized at the response edge.</summary>
    public string? MessageCode { get; init; }
    /// <summary>Arguments for the localized text placeholders ({0}, {1}, ...).</summary>
    ...
    // 'code' is the HTTP status; 'messageCode' is the translation key resolved per request language.
...
public record BaseResult<T> : BaseResult
{
    ...
    public BaseResult(BaseResult result, T? data) : base(result) => Data = data;
    protected BaseResult(ResultType type, string code, T? data, string? message = null, Dictionary<string, string>? values = null)
        : base(type, code, message, values) => Data = data;
